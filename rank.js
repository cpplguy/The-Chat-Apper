...
export const getRankBadge = (rank, isUser) => {
  const r = RANKS[rank] ?? RANKS.member;
  const rankColor = r?.color || "gray";
  const rankBadge = r?.badge || "👤";
  const rankLabel = r?.label || "Member";
  return <span className = "rank" style={{color:rankColor, fontSize:"0.7rem", fontWeight:"bold", border:`1px solid ${rankColor}`, borderRadius:"9px", padding:"1px 6px", marginRight: "1vh"}}>{rankBadge} {rankLabel}</span>;
};
